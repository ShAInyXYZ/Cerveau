...
run('node',['--test','internal/tools/browsernative/engine.test.mjs']);
run('python3',['-m','unittest','discover','-s','sidecars','-p','test_*.py']);
...
run('node',['--test','scripts/qa-panel-css.test.mjs']);
...
run('go',['build','-trimpath','-o',join(dest,'ignite'),'./sidecars/ignite']);
const planner=plannerFromBinary(JSON.parse(output(join(dest,'crv'),['-build-info'])),version,revision);
const manifest={version,codename:'LABRIG',revision,head,dirty,source_sha256:sourceSHA256,built_at:new Date().toISOString(),go:output('go',['version']),node:process.version,planner_version:planner.version,planner,deployed:false};
writeFileSync(join(dest,'build.json'),JSON.stringify(manifest,null,2)+'\n');
const sums=releaseArtifacts.map(file=>`${createHash('sha256').update(readFileSync(join(dest,file))).digest('hex')}  ${file}`).join('\n')+'\n';
