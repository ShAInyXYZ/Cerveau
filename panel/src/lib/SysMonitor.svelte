...
<div class="mon">
  ...
  <button class="cluster" aria-label="system status" aria-expanded={expanded} popovertarget={popoverID}>
    {#if gpu}
...
<style>
  ...
  .pop {
    ...
    width: min(360px, calc(100vw - 32px)); max-height: calc(100dvh - 80px); overflow-y: auto;
    z-index: var(--z-popover); color: var(--text); border: 1px solid var(--line2);
    background: var(--floating-surface); backdrop-filter: var(--floating-blur); border-radius: var(--r-panel); box-shadow: var(--elev-2);
    padding: 6px;
